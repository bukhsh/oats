...
# --- ZIP load model ---
def real_load(model,d):
    return model.pD[d] == sum(0.0*model.v[b]**2+0.0*model.v[b]+1.0 for b in model.B if (b,d) in model.Dbs)*model.PD[d]
    # return model.pD[d] == sum(0.29*model.v[b]**2+0.1*model.v[b]+0.61 for b in model.B if (b,d) in model.Dbs)*model.PD[d]
def reactive_load(model,d):
    return model.qD[d] == sum(0.0*model.v[b]**2-0.0*model.v[b]+1.0 for b in model.B if (b,d) in model.Dbs)*model.QD[d]
    # return model.qD[d] == sum(3.22*model.v[b]**2-4.53*model.v[b]+2.31 for b in model.B if (b,d) in model.Dbs)*model.QD[d]
model.RealLoad     = Constraint(model.D, rule=real_load)
model.ReactiveLoad = Constraint(model.D, rule=reactive_load)
